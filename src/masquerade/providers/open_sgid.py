    ...
    def get_magic_key_record(self, query):
        ...
        with pool.connection() as connection:
            with connection.cursor() as cursor:
                ...
                return (
                    cursor.fetchone(),
                    [desc[0] for desc in cursor.description if desc[0] != "shape"],
                )
...
class Table:
    ...
    def make_suggestion(self, xid, suggestion_text):
        ...
        return {
            "isCollection": False,
            "magicKey": f"{xid}{self.SPLITTER}{self.table_name}",
    ...
    def get_candidate_from_magic_key(self, xid, out_spatial_reference):
        ...
        return {
            "address": match_text,
            "score": 100,
            "location": {"x": x_value, "y": y_value},
            "attributes": attributes,
            "extent": {"xmax": xmax, "xmin": xmin, "ymax": ymax, "ymin": ymin},
        }
...
TABLES = [
    ...
    AddressPointTable(
        ...
        FULLADD,
        POINT,
        additional_out_fields=[ADDSYSTEM, CITY],
    ),
    Table(
